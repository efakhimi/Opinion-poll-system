...
@section('subhead')
    <title>سامانه نظرسنجی | ویرایش پرسشنامه</title>
@endsection

@section('subcontent')
    <div class="flex items-center mt-8">
        <h2 class="intro-y text-lg font-medium ml-auto">ویرایش پرسشنامه : {{$surveyData['title']}}</h2>
    </div>
@if ($errors->any())
    ...
    <div class="intro-y box py-5 sm:py-5 mt-5">
        <div class="px-5 sm:px-20 mt-0">
            ...
            <div class="grid grid-cols-12 gap-4 gap-y-5 mt-5">
                ...
                <form name="myform" novalidate  method="post" action="{{url('edit-survey/'.$surveyData['id'])}}" enctype="multipart/form-data">
                @csrf
        <div class="px-5 sm:px-20 mt-10 pt-10 border-t border-slate-200/60 dark:border-darkmode-400">
            ...
            <div class="grid grid-cols-12 gap-4 gap-y-5 mt-5">
                <div class="intro-y col-span-12 sm:col-span-6">
                    ...
                    <input id="input-wizard-1" type="text" class="form-control" name="title" placeholder="عنوان" required  value="{{ old('title') ? old('title') : $surveyData['title'] }}">
                </div>
                <div class="intro-y col-span-12 sm:col-span-6">
                ...
                <div class="intro-y col-span-12 sm:col-span-12">
                    ...
                    <textarea class="form-control editor" name="desc" placeholder="توضیحات پرسشنامه" minlength="10">{{ old('desc') ? old('desc') : $surveyData['description'] }}</textarea>
                </div>
                <div class="intro-y col-span-12 sm:col-span-3">
                    <label for="input-wizard-4" class="form-label">* در دسترس باشد؟</label>
                    <div class="form-switch mt-2">
                        <input type="checkbox" class="form-check-input" name="active" {{ ( $surveyData['active']==1 ? "checked" : "" )}}>
                    </div>
                </div>
                <div class="intro-y col-span-12 sm:col-span-3">
                    <label for="input-wizard-4" class="form-label">* عمومی باشد؟</label>
                    <div class="form-switch mt-2">
                        <input type="checkbox" class="form-check-input" name="public" {{ ( $surveyData['public']==1 ? "checked" : "" )}}>
                    </div>
                </div>
                <div class="intro-y col-span-12 sm:col-span-3">
                    <label for="input-wizard-4" class="form-label">* فقط کاربران ثبت نام شده؟</label>
                    <div class="form-switch mt-2">
                        <input type="checkbox" class="form-check-input" name="registered" {{ ( $surveyData['registered_only']==1 ? "checked" : "" )}}>
                    </div>
                </div>
                <div class="intro-y col-span-12 sm:col-span-3">
                    <label for="input-wizard-4" class="form-label">* نوع پرسشنامه</label>
                    <select id="input-wizard-6" class="form-select" name="type">
                        <option value=0 {{ ( $surveyData['type']==0 ? "selected" : "" )}}>معمولی چند سوالی</option>
                        <option value=1 {{ ( $surveyData['type']==1 ? "selected" : "" )}}>امتحان</option>
                        <option value=2 {{ ( $surveyData['type']==2 ? "selected" : "" )}}>معمولی تک سوالی</option>
                    </select>
                </div>
